...
import { useApplyApplication, useLoggedInStudentApplications } from "../../hooks/useApplication.js";
...
export default function CompanyListings() {
  ...
  const isApplied = (companyId) =>
    applications.some(
      (a) => a.student?._id === currentStudentData?.data?._id && a.company?._id === companyId,
    );

  // ← check if student is eligible
  const isEligible = (co) => (currentStudentData?.data?.cgpa || 0) >= co.minCGPA;

  const daysLeft = (deadline) =>
    Math.ceil((new Date(deadline).getTime() - Date.now()) / 86400000);

  // These are all filtered Compaies  !!
  // By Default all Companies Showed !!
  const filtered = [...companies]
    .filter((c) => {
      ...
      const matchEligible =
        !showEligible || (currentStudentData?.data?.cgpa || 0) >= c.minCGPA;
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Browse and apply to placement drives visiting JMIT campus
          </p>
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {filtered.map((co) => {
              const applied = isApplied(co._id);
              const eligible = isEligible(co);
              const days = daysLeft(co.deadline);
              const gradient =
                COMPANY_COLORS[co.name] || "from-blue-400 to-blue-600";
              return (
                <div
                  key={co.name}
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-12 h-12 rounded-xl bg-linear-to-r ${gradient} flex items-center justify-center text-white font-bold text-lg shadow-md`}
                        >
                          {co.logo}
                        </div>
                        <div>
                      ...
                      <span
                        className={`text-xs font-medium px-2.5 py-1 rounded-full border ${STATUS_STYLES[co.status]}`}
                      >
                        {co.status}
                      </span>
                    </div>
                    ...
                    >
                      ...
                      {days < 0
                        ...
                        : days === 0
                          ? "Deadline: Today!"
                          : `Deadline: ${days} days left (${co.deadline})`}
                    </div>
                    ...
                    {!eligible && (
                      <div className="flex items-center gap-2 text-xs text-amber-600 bg-amber-50 px-3 py-2 rounded-lg mb-3">
                        ...
                        Your CGPA ({currentStudentData?.data?.cgpa}) is below minimum (
                        {co.minCGPA})
                      </div>
                    )}
                    ...
                    <div className="flex gap-2">
                      ...
                      {applied ? (
                        ...
                        >
                          ...
                          {appliedId === co._id ? "Applied!" : "Apply"}
                        </button>
                      )}
                    </div>
                  </div>
      ...
      {selectedCompany && (
        ...
        >
          ...
          >
            <div
              className={`h-24 bg-linear-to-r ${COMPANY_COLORS[selectedCompany.name] || "from-blue-400 to-blue-600"} relative`}
            >
              <button
                onClick={() => setSelectedCompany(null)}
            ...
            <div className="px-6 pb-6">
              <div className="flex items-end gap-4 -mt-8 mb-5">
                <div
                  className={`w-16 h-16 mt-9 rounded-2xl bg-linear-to-br ${COMPANY_COLORS[selectedCompany.name] || "from-blue-400 to-blue-600"} flex items-center justify-center text-white font-bold text-xl shadow-lg border-4 border-white`}
                >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    {
                      label: "Package",
                      value: `${selectedCompany.packageLPA} LPA`,
                    },
                    {
                      label: "Min CGPA",
                      value: selectedCompany.minCGPA.toString(),
                    },
                    {
                      label: "Openings",
                      value: `${selectedCompany.positions} positions`,
                    },
                ...
                {!isEligible(selectedCompany) && (
                  <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 flex items-center gap-2">
                    ...
                    <p className="text-sm text-amber-700">
                      Your CGPA ({currentStudentData?.data?.cgpa}) is below the minimum
                      requirement ({selectedCompany.minCGPA})
                    </p>
                  </div>
                )}

                {/* Apply Button */}
                {isApplied(selectedCompany._id) ? (
                  ...
                  >
                    ...
                    {selectedCompany.status === "Active"
                      ? "Apply Now"
                      : selectedCompany.status === "Upcoming"
                        ? "Opening Soon"
                        : "Applications Closed"}
                  </button>
                )}
