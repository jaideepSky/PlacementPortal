...
import {
  ...
  MapPin, TrendingUp, Users, Award, Clock, CheckCircle,
  AlertTriangle, MoreVertical, Filter, Briefcase
} from "lucide-react"
import { useSelector, useDispatch } from "react-redux"   // ← redux
import { addCompany, deleteCompany, updateCompany } from '../../redux/slices/companySlice.js'  // ← redux
import { nanoid } from "nanoid"

const INDUSTRY_OPTIONS = ["Technology", "IT Services", "E-Commerce", "Software", "Consulting", "E-Commerce & Cloud", "Finance", "Healthcare", "Manufacturing"]
...
const emptyForm = {
  name: "", logo: "", industry: "Technology", location: "",
  packageLPA: 0, positions: 0, minCGPA: 0, deadline: "",
  description: "", status: "Active", jobRole: "", requirements: [],
}
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white z-10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-semibold text-gray-900">{initial ? "Edit Company" : "Add New Company"}</h2>
          </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <h3 className="font-semibold text-gray-900 text-center mb-2">Remove Company?</h3>
        <p className="text-sm text-gray-500 text-center mb-6">
          Are you sure you want to remove <strong>{company.name}</strong>? This action cannot be undone.
        </p>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-gray-600 font-medium hover:bg-gray-50 text-sm">Cancel</button>
          <button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl bg-red-600 text-white font-medium hover:bg-red-700 transition-colors text-sm">Remove</button>
        </div>
      </div>
    </div>
  )
...
export default function CompanyManagement() {
  ...
  const { companiesList }            = useSelector(state => state.companies)            // ← redux
  ...
  // ← dispatch addCompany instead of setCompanies
  ...
  // ← dispatch updateCompany instead of setCompanies
  ...
  // ← dispatch deleteCompany instead of setCompanies
  const handleDelete = () => {
    if (!deleteComp) return
    dispatch(deleteCompany({ id: deleteComp.id }))
    showToast(`${deleteComp.name} removed.`)
    setDeleteComp(null)
  }

  // ← dispatch updateCompany for status change
  ...
  const stats = [
    ...
    { label: "Active Drives",   value: companiesList.filter(c => c.status === "Active").length,     icon: TrendingUp, color: "text-green-600", bg: "bg-green-50"  },
    { label: "Upcoming",        value: companiesList.filter(c => c.status === "Upcoming").length,   icon: Clock,     color: "text-amber-600",  bg: "bg-amber-50"  },
    { label: "Total Positions", value: companiesList.reduce((acc, c) => acc + c.positions, 0),      icon: Briefcase, color: "text-purple-600", bg: "bg-purple-50" },
  ]
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Add, edit and manage placement drives visiting JMIT</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          {filtered.length === 0 ? (
            <div className="p-16 text-center">
              ...
              <h3 className="font-semibold text-gray-500 mb-1">No companies found</h3>
              <p className="text-sm text-gray-400">
                {search || statusFilter !== "All" ? "Try adjusting your search" : "Add your first company!"}
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    <th className="text-left px-6 py-3.5 text-xs text-gray-500 font-semibold uppercase tracking-wider">Company</th>
                    <th className="text-left px-4 py-3.5 text-xs text-gray-500 font-semibold uppercase tracking-wider">Role & Industry</th>
                    <th className="text-left px-4 py-3.5 text-xs text-gray-500 font-semibold uppercase tracking-wider">Package</th>
                    <th className="text-left px-4 py-3.5 text-xs text-gray-500 font-semibold uppercase tracking-wider">Positions</th>
                    <th className="text-left px-4 py-3.5 text-xs text-gray-500 font-semibold uppercase tracking-wider">Deadline</th>
                    <th className="text-left px-4 py-3.5 text-xs text-gray-500 font-semibold uppercase tracking-wider">Status</th>
                    <th className="text-right px-6 py-3.5 text-xs text-gray-500 font-semibold uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={co.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-4">
                          <p className="font-medium text-gray-800">{co.jobRole}</p>
                        ...
                        <td className="px-4 py-4">
                          ...
                          <p className="text-xs text-gray-400">Min CGPA: {co.minCGPA}</p>
                        </td>
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-1">
                            <Users className="w-3.5 h-3.5 text-indigo-400" />
                            <span className="text-gray-700">{co.positions}</span>
                          </div>
                        </td>
                        <td className="px-4 py-4">
                          <p className="text-gray-700 text-xs">{co.deadline}</p>
                          <p className={`text-xs font-medium mt-0.5 ${days < 0 ? "text-gray-400" : days <= 7 ? "text-red-500" : "text-amber-500"}`}>
                            {days < 0 ? "Passed" : `${days}d left`}
                          </p>
                        </td>
                        <td className="px-4 py-4">
                          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${STATUS_STYLES[co.status]}`}>
                            {co.status}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-end gap-1 relative">
                            <button onClick={() => setEditCompany(co)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                              <Edit3 className="w-4 h-4" />
                            </button>
                            <button onClick={() => setDeleteComp(co)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                            ...
                            <div className="relative">
                              ...
                              {menuOpen === co.id && (
                                <div className="absolute right-0 mt-1 w-44 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-20">
                                  <p className="px-3 py-1.5 text-xs text-gray-400 font-semibold uppercase tracking-wider">Change Status</p>
                                  {STATUS_OPTIONS.map(s => (
                                    <button key={s} onClick={() => handleStatusChange(co.id, s)}
                                      ...
                                      {co.status === s && <CheckCircle className="w-3.5 h-3.5" />}
                                      {s}
                                    </button>
                                  ))}
                                </div>
      ...
      {/* Modals */}
      {showForm    && <CompanyFormModal onSave={handleAdd} onClose={() => setShowForm(false)} />}
      {editCompany && <CompanyFormModal initial={editCompany} onSave={handleEdit} onClose={() => setEditCompany(null)} />}
      {deleteComp  && <DeleteConfirmModal company={deleteComp} onConfirm={handleDelete} onClose={() => setDeleteComp(null)} />}
    </div>
  )
}