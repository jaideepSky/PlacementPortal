...
import {
  Users, Search, Filter, ChevronDown, Eye, Edit3,
  CheckCircle, Clock, XCircle, TrendingUp, Calendar,
  Briefcase, Award, X, Save, GraduationCap, Phone,
  Mail, Building, Star, ArrowUpDown, LayoutGrid, List
} from "lucide-react"
import { useSelector, useDispatch } from "react-redux"          // ← redux
import { updateStatus } from "../../redux/slices/applicationSlice.js"  // ← redux

const STATUS_CONFIG = {
  "Applied":             { color: "#3b82f6", bg: "bg-blue-50",   icon: <Clock className="w-4 h-4" /> },
  "Shortlisted":         { color: "#f59e0b", bg: "bg-amber-50",  icon: <TrendingUp className="w-4 h-4" /> },
  "Interview Scheduled": { color: "#8b5cf6", bg: "bg-purple-50", icon: <Calendar className="w-4 h-4" /> },
  "Selected":            { color: "#10b981", bg: "bg-green-50",  icon: <CheckCircle className="w-4 h-4" /> },
  "Rejected":            { color: "#ef4444", bg: "bg-red-50",    icon: <XCircle className="w-4 h-4" /> },
}

const STATUSES      = ["Applied", "Shortlisted", "Interview Scheduled", "Selected", "Rejected"]
const ROUND_OPTIONS = ["", "Aptitude Test", "Technical Round 1", "Technical Round 2", "HR Round", "Offer Extended"]
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">{app.studentName} → {app.companyName}</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-between px-4 py-3 rounded-xl border-2 transition-all text-left ${
                      status === s ? `${cfg.bg}` : "border-gray-100 hover:border-gray-200 hover:bg-gray-50"
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      <span style={{ color: cfg.color }}>{cfg.icon}</span>
                      <span className="text-sm font-medium" style={{ color: cfg.color }}>{s}</span>
                    </div>
                    {status === s && <CheckCircle className="w-4 h-4" style={{ color: cfg.color }} />}
                  </button>
                )
              })}
            </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Current Round (optional)</label>
            <select
              value={round}
          ...
          <div className="flex gap-3">
            <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-gray-600 font-medium hover:bg-gray-50 text-sm">
              Cancel
            </button>
            <button
...
function StudentDetailModal({ student, apps, onClose }) {
  const studentApps = apps.filter(a => a.studentId === student.id)
        ...
        <div className="bg-linear-to-r from-blue-700 to-indigo-700 p-6 relative">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-blue-200 text-sm">{student.department} • {student.year}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-blue-50 rounded-xl p-3 text-center">
              <p className="text-lg font-bold text-blue-700">{student.cgpa?.toFixed(1)}</p>
              <p className="text-xs text-gray-500">CGPA</p>
            </div>
            <div className="bg-amber-50 rounded-xl p-3 text-center">
              <p className="text-lg font-bold text-amber-700">{studentApps.length}</p>
              <p className="text-xs text-gray-500">Applied</p>
            </div>
            <div className="bg-green-50 rounded-xl p-3 text-center">
              <p className="text-lg font-bold text-green-700">{selected.length}</p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {student.skills.map(skill => (
                  <span key={skill} className="bg-blue-50 text-blue-700 border border-blue-100 px-3 py-1 rounded-full text-xs font-medium">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          {studentApps.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-2">Applications ({studentApps.length})</h3>
              <div className="space-y-2">
                {studentApps.map(app => (
                  <div key={app.id} className="flex items-center justify-between bg-gray-50 rounded-xl p-3">
                    <div>
                      <p className="text-sm font-medium text-gray-800">{app.companyName}</p>
                      <p className="text-xs text-gray-400">{app.jobRole} • {app.packageLPA} LPA</p>
                    </div>
          ...
          {selected.length > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-sm font-semibold text-green-800">Placement Offer</p>
              </div>
              {selected.map(s => (
...
export default function StudentApplicationManagement() {
  ...
  const { applicationsList: applications } = useSelector(state => state.applications)  // ← redux
  const { studentsList: students }         = useSelector(state => state.students)       // ← redux
  ...
  const companyNames = ["All", ...Array.from(new Set(applications.map(a => a.companyName))).sort()]
  const departments  = ["All", ...Array.from(new Set(students.map(s => s.department || ""))).filter(Boolean).sort()]
  ...
  const summaryCards = [
    { label: "Total Applications", value: applications.length,           bg: "bg-blue-50",  color: "text-blue-600",  icon: Briefcase  },
    { label: "Shortlisted",        value: statusCounts["Shortlisted"] || 0, bg: "bg-amber-50", color: "text-amber-600", icon: TrendingUp },
    { label: "Selected",           value: statusCounts["Selected"]    || 0, bg: "bg-green-50", color: "text-green-600", icon: CheckCircle },
    { label: "Pending",            value: statusCounts["Applied"]     || 0, bg: "bg-gray-50",  color: "text-gray-600",  icon: Clock      },
  ]
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Manage and update JMIT student application statuses</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {summaryCards.map(card => (
            <div key={card.label} className={`${card.bg} rounded-2xl p-4 flex items-center gap-3`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-6 shadow-sm">
          <div className="flex flex-col lg:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search by student, company, roll no..."
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <div className="relative">
                <select value={companyFilter} onChange={e => setCompanyFilter(e.target.value)}
                  className="appearance-none text-sm border border-gray-200 rounded-xl pl-3 pr-8 py-2.5 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
                  {companyNames.map(c => <option key={c} value={c}>{c === "All" ? "All Companies" : c}</option>)}
                </select>
                <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none" />
              </div>
              <div className="relative">
              ...
              <button
                onClick={() => setSortBy(sortBy === "date" ? "name" : sortBy === "name" ? "cgpa" : "date")}
                ...
                Sort: {sortBy === "date" ? "Date" : sortBy === "name" ? "Name" : "CGPA"}
              </button>
            </div>
          </div>
          <p className="text-xs text-gray-400 mt-2">{filtered.length} applications</p>
        </div>
        ...
        {/* card View */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.length === 0 ? (
              ...
              filtered.map(app => (
                <div key={app.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden">
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">{app.studentRoll} • CGPA {app.cgpa}</p>
                        </div>
                      </div>
                      <StatusBadge status={app.status} />
                    </div>
                    <div className="bg-gray-50 rounded-xl p-3 mb-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-semibold text-gray-800 text-sm">{app.companyName}</p>
                          <p className="text-xs text-gray-500">{app.jobRole}</p>
                        </div>
                        <span className="text-green-600 font-bold text-sm">{app.packageLPA} LPA</span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between text-xs text-gray-400 mb-4">
                      ...
                      <span className="flex items-center gap-1">
                        <Building className="w-3.5 h-3.5" /> {app.department}
                      </span>
                      {app.round && <span className="text-purple-500 font-medium">{app.round}</span>}
                    </div>
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setUpdateApp(app)}
      ...
      {/* Modals */}
      {updateApp   && <UpdateStatusModal app={updateApp} onSave={handleStatusUpdate} onClose={() => setUpdateApp(null)} />}
      {viewStudent && <StudentDetailModal student={viewStudent} apps={applications} onClose={() => setViewStudent(null)} />}
